...
class BaseProvider():
    ...
    def compare_transactions(self, account: "Account"):
        ...
        for cu_t in current_transactions:
            ...
            try:
                matching_index = next(index for index, ca_t in enumerate(cached_transactions) if ca_t.matches(cu_t))
        ...
        matches = [cu_t for cu_t, ca_t in transaction_mapping] # Take the cu_t from the mapping given that even if they match, cu_t is fresh
